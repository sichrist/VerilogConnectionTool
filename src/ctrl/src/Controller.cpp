...
rsync (map < string, string > *filelist, string dstfiles)
{
  ...
  for (std::map < string, string >::iterator it = filelist->begin ();
       it != filelist->end (); ++it)
    {
...
Connection *
Controller::buildConnectionObject (string inst1, string inst2, string con,
				   string projpath)
{
  ...
  Connection *connection_ = new Connection (mod, connection, inst1, inst2,
					    projectpath +"py/vparser.py");
